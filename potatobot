  ...
  discord.on(discord.Event.MESSAGE_CREATE, async (message: discord.Message) => {
    ...
    if (await potatoKV.get<boolean>('cooldown')) {
      if (message.content === discord.decor.Emojis.POTATO) {
        ...
        const poisonous = Math.random() < 0.3;
        ...
        await message.reply(
          new discord.Embed({
            ...
            description: `${message.author.getTag()} ${
              poisonous
                ? `tried to pick up a poisonous potato, poisoning your nuts in the process`
            ...
            footer: {
              text: poisonous
                ...
                : 'Imagine not being fast enough to claim the potatoes, could not be me!',
            },
          })
        );
      }
    ...
    const reply = await message.reply(
      new discord.Embed({
        ...
        description:
          'Type in a `potato` to claim it before someone else gets it!',
        color: 0x873f16,
      })
    );
  ...
  potatoCommands.subcommand('potato', (potatoSubcommands) => {
      ...
      async (message) => {
        ...
        await message.inlineReply(
          new discord.Embed({
            ...
            description: [
              `When a ${discord.decor.Emojis.POTATO} is dropped, be the first to pick it up by posting a potato emoji`,
              '',
              'Example command:',
              ...
              '- `!potato work` - work for some potatos, maybe you will get some',
              '- `!potato hourly` - get some potatos every hour',
              '- `!potato daily` - get some potatos every 24 hours',
              '_ `!potato weekly` - get some potatos every week',
              '',
    ...
    potatoSubcommands.on(
      ...
      async (message, { who, count }) => {
        ...
        if (count > 30)
          return await message.inlineReply(
            '❌ Your balls can only carry 30 potatos! try again!'
          );
  
        await potatoKV.put(`steal-${message.author?.id}`, true, {
          ttl: randomTimeBetween(1 * 60 * 1000, 10 * 60 * 1000),
        });
        ...
        await message.inlineReply(
          new discord.Embed({
            ...
            description: `Your thievery ${success ? 'winning' : 'died'}, you ${
              success ? 'gottem' : 'returned'
            } ${count} potato${count === 1 ? '' : 's'} ${
              success ? 'from' : 'to'
            } ${who.getTag()}, ${
              success ? 'giving you a total of' : 'leaving you with'
            } ${newUserPotatos} potatos :potato: LOL${
              newUserPotatos === 1 ? '' : 's'
            }`,
            color: 0x11111c,
          })
    ...
    potatoSubcommands.on(
      ...
      async (message) => {
        ...
        const reply = await message.reply(
          new discord.Embed({
            title: 'A potato has been dropped!',
            description: 'Type in a `potato` to claim it!',
            color: 0x873f16,
          })
        );
        const cooldown = randomTimeBetween(3 * 60 * 1000, 20 * 60 * 1000);
    ...
    potatoSubcommands.on(
      { name: 'work', description: 'work for potatos' },
      () => ({}),
      async (message) => {
        if (!message.author || message.author.bot) return;
        if (await potatoKV.get<boolean>(`begcmd-${message.author.id}`))
          return await message.inlineReply(
            '⚠️Stop working so much, your boss is going to be poor'
          );
        await potatoKV.put(`begcmd-${message.author.id}`, true, {
          ttl: Math.ceil(Date.now() / 1000 / 30) * 30 * 1000 - Date.now(),
        });
        let amount = Math.floor(Math.random() * 7);
        if (amount === 0) {
          return await message.inlineReply(
            '❌ You did not get anything from working this time, maybe you should try again later?'
          );
        }
        ...
        await message.inlineReply(
          `✅ You got **${amount}** potato(s) from working, and now hold onto **${newCount}** potatos!`
        );
      }
    );
  
    potatoSubcommands.on(
      { name: 'ezget', description: 'get for potatos' },
      () => ({}),
      async (message) => {
        if (!message.author || message.author.bot) return;
        if (await potatoKV.get<boolean>(`getcmd-${message.author.id}`))
          return await message.inlineReply(
            'bro wtf are you doing the cooldown is LITERLALY ONE SECOND'
          );
        await potatoKV.put(`getcmd-${message.author.id}`, true, {
          ttl: Math.ceil(Date.now() / 1000 / 1) * 1 * 1000 - Date.now(),
        });
        let amount = Math.floor(Math.random() * 42062349);
        if (amount === 0) {
          return await message.inlineReply(
            '❌ You did not get anything from getting this time, maybe you suck nuts??'
          );
        }
        const newCount = await potatoKV.transact(
          message.author.id,
          (prev: number | undefined) => (prev || 0) + amount + 420694300
        );
        await message.inlineReply(
          `✅ You got **${amount}** potato(s) from getting, and now hold onto **${newCount}** potatos!`
        );
      }
    );
    ...
    potatoSubcommands.on(
      { name: 'weekly', description: 'weekie potato' },
      () => ({}),
      async (message) => {
        if (!message.author || message.author.bot) return;
        if (await potatoKV.get<boolean>(`weekly-${message.author.id}`)) {
          return await message.inlineReply(
            new discord.Embed({
              description: `❌ You have already claimed your weekly potatoes! You think I am made out of potatos?`,
              color: 0xff0000,
              footer: { text: 'Ah yes, weekly potatoes do not be going brrr' },
            })
          );
        }
        await potatoKV.put(`weekly-${message.author.id}`, true, {
          ttl:
            Math.ceil(Date.now() / 1000 / 60 / 60 / 24 / 6.7) *
              6.7 *
              24 *
              60 *
              60 *
        ...
        await message.inlineReply(
          new discord.Embed({
            description: `✅ You claimed **${amount}** potatos as your weekly reward, and now hold onto **${newCount}** potatos!`,
            color: 0x00ff00,
            footer: { text: 'Ah yes, weekly potatoes do be going brrr' },
          })
        );
      }
    );
    ...
    const SHOP_ITEMS = {
      ...
      notScam: {
        price: 30,
        description: '`notScam` Buy 20 potatoes with 30 potatoes...',
        async onPurchase(user: discord.User) {
      ...
      deezNuts: {
        price: 9999999,
        description:
          '`deezNuts` Get deez nuts in a box! Comes within 1-2 days, with free shipping!',
        async onPurchase(user: discord.User) {},
        enabled: true,
        duration: 24 * 60 * 60 * 1000, // 24 hours, checked in 5 minute intervals
      },
